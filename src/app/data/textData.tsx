export const mobileMenuData = [
  {
    en: "Home",
    es: "Inicio",
    href: "#home",
  },
  {
    en: "About Us",
    es: "Nosotros",
    href: "#about",
  },
  {
    en: "Services",
    es: "Servicios",
    href: "#services",
  },
  {
    en: "Fleet",
    es: "Flota",
    href: "#fleet",
  },
  {
    en: "Careers",
    es: "Carreras",
    href: "#careers",
  },
  {
    en: "Contact Us",
    es: "Contáctenos",
    href: "#contact",
  },
];

export const aboutText = {
  es: {
    title: "Sobre Nosotros",
    description:
      "Lorem ipsum dolor sit amet consectetur adipiscing elit volutpat, lacinia proin a quisque imperdiet himenaeos donec, mi massa risus morbi vitae neque sodales. Pellentesque pretium tincidunt id tellus lacinia vulputate interdum eros euismod etiam, mi mollis luctus odio in quam mus metus platea rutrum, sodales et hac felis dis nunc sociis sollicitudin primis. Hendrerit quis mi tortor sollicitudin curabitur aptent ante lacus in sed egestas, porttitor eu dignissim senectus taciti cum justo mattis aliquam nunc. Aenean odio himenaeos fames proin, elementum ante dignissim lacus semper, habitasse accumsan iaculis. Tempus morbi a ultricies dictum vel, pulvinar facilisi viverra sodales, ut quis cum massa. Placerat montes quisque vestibulum nullam augue primis cum curabitur, odio scelerisque taciti justo ut libero dapibus parturient in, platea nibh etiam vitae tristique turpis nulla.",
    history:
      "Lorem ipsum dolor sit amet consectetur adipiscing elit volutpat, lacinia proin a quisque imperdiet himenaeos donec, mi massa risus morbi vitae neque sodales. Pellentesque pretium tincidunt id tellus lacinia vulputate interdum eros euismod etiam, mi mollis luctus odio in quam mus metus platea rutrum, sodales et hac felis dis nunc sociis sollicitudin primis. Hendrerit quis mi tortor sollicitudin curabitur aptent ante lacus in sed egestas, porttitor eu dignissim senectus taciti cum justo mattis aliquam nunc.",
    values: [
      {
        name: "Eficiencia",
        description: "Entrega de su carga de manera rápida y puntual.",
      },
      {
        name: "Confiabilidad",
        description:
          "Aseguramos que sus envíos lleguen a su destino de manera segura.",
      },
      {
        name: "Experiencia",
        description:
          "Aprovechamos nuestro conocimiento y experiencia en la industria para manejar su carga con cuidado.",
      },
      {
        name: "Enfoque en el cliente",
        description:
          "Priorizamos sus necesidades y brindamos un servicio al cliente excepcional.",
      },
    ],
  },
  en: {
    title: "About Us",
    description:
      "Lorem ipsum dolor sit amet consectetur adipiscing elit volutpat, lacinia proin a quisque imperdiet himenaeos donec, mi massa risus morbi vitae neque sodales. Pellentesque pretium tincidunt id tellus lacinia vulputate interdum eros euismod etiam, mi mollis luctus odio in quam mus metus platea rutrum, sodales et hac felis dis nunc sociis sollicitudin primis. Hendrerit quis mi tortor sollicitudin curabitur aptent ante lacus in sed egestas, porttitor eu dignissim senectus taciti cum justo mattis aliquam nunc. Aenean odio himenaeos fames proin, elementum ante dignissim lacus semper, habitasse accumsan iaculis. Tempus morbi a ultricies dictum vel, pulvinar facilisi viverra sodales, ut quis cum massa. Placerat montes quisque vestibulum nullam augue primis cum curabitur, odio scelerisque taciti justo ut libero dapibus parturient in, platea nibh etiam vitae tristique turpis nulla.",
    history:
      "Lorem ipsum dolor sit amet consectetur adipiscing elit volutpat, lacinia proin a quisque imperdiet himenaeos donec, mi massa risus morbi vitae neque sodales. Pellentesque pretium tincidunt id tellus lacinia vulputate interdum eros euismod etiam, mi mollis luctus odio in quam mus metus platea rutrum, sodales et hac felis dis nunc sociis sollicitudin primis. Hendrerit quis mi tortor sollicitudin curabitur aptent ante lacus in sed egestas, porttitor eu dignissim senectus taciti cum justo mattis aliquam nunc.",
    values: [
      {
        name: "Efficiency",
        description: "Delivering your cargo quickly and on time.",
      },
      {
        name: "Reliability",
        description:
          "Ensuring your shipments reach their destination safely and securely.",
      },
      {
        name: "Expertise",
        description:
          "Leveraging our industry knowledge and experience to handle your cargo with care.",
      },
      {
        name: "Customer Focus",
        description:
          "Prioritizing your needs and providing exceptional customer service.",
      },
    ],
  },
};

export const SocialData = [
  {
    platform: "Instagram",
    icon: "instagram",
    link: "https://www.instagram.com/",
  },
  {
    platform: "Facebook",
    icon: "facebook",
    link: "https://www.facebook.com/",
  }
];
